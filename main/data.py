...
def load_data(data_cfg: dict) -> (Dataset, Dataset, Dataset, Vocabulary):
    """
    ...
    train_data = SignTranslationDataset(
        path=train_paths,
        fields=(sequence_field, signer_field, sgn_field, txt_field),
...
def token_batch_size_fn(new, count, sofar):
    ...
    return max(sgn_elements, txt_elements)
...
def make_data_iter(
    ...
    :param train: whether it's training time, when True, returns a BucketIterator
    ...
    if train:
        ...
        # no need to shuffle/sort for inference
        data_iter = data.BucketIterator(
            repeat=False,
            dataset=dataset,
            batch_size=batch_size,
