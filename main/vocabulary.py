...
def build_vocab(
    ...
    :param field: attribute name of the dataset to build vocabulary for
    ...
    for special in vocab.specials:
        if special not in vocab.stoi:
            if special == vocab.itos[vocab.DEFAULT_UNK_ID()]:
                continue
            else:
                raise ValueError(
                    "Special token {} not in vocab. "
                    "Vocab is {}.".format(special, vocab.itos)
                )
    return vocab
