...
def load_config(path="configs/default.yaml") -> dict:
    ...
    If model_dir contains "auto", it will be replaced with current timestamp.
    ...
    if "training" in cfg and "model_dir" in cfg["training"]:
        ...
        if "auto" in model_dir:
            # Generate timestamp in format YYYYMMDD_HHMMSS
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            # Replace "auto" with timestamp
            cfg["training"]["model_dir"] = model_dir.replace("auto", timestamp)
...
def freeze_params(module: nn.Module):
    ...
    i.e. turn off gradient computation.
...
def symlink_update(target, link_name):
    """
    Find the file to which the symbolic link points, similar to
    os.readlink, but it throws an error if the link is broken.
