...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, Q, K, V, mask=None):
        ...
        # Apply mask
        if mask is not None:
            att_scores = att_scores.masked_fill(~mask.unsqueeze(1), float("-inf"))
...
class ContextualMultiHeadAttention(nn.Module):
    def __init__(self, dim_model, num_heads):
        super(ContextualMultiHeadAttention, self).__init__()
        ...
        self.position_encodings = nn.ModuleList(
            [CoPE(5000, self.dim_head) for _ in range(num_heads)]
        )
    ...
    def forward(self, Q, K, V, mask=None):
        ...
        # Apply mask
        if mask is not None:
            att_scores += mask.log().unsqueeze(1)
        for i in range(self.num_heads):
            att_scores[:, i, :, :] += self.position_encodings[i](Q[:, i, :, :], att_scores[:, i, :, :])
...
class RelPosMultiHeadSelfAttention(MultiHeadAttention):
    ...
    def __init__(self, dim_model, num_heads, causal, max_pos_encoding):
        ...
        #self.cope = CoPE(5000, self.dim_model)
...
class DeformableMultiHeadedAttention(nn.Module):
    ...
    def __init__(
        ...
        self.attention_scores = None  # Save attention scores for visualization
        self.cope = cope

        if self.cope:
            self.position_encodings = nn.ModuleList([CoPE(5000, self.head_size) for _ in range(num_heads)])
    ...
    def forward(self, k, v, q, mask = None):
        ...
        q_2 = q.unsqueeze(3).expand(
            batch_size, num_heads, query_len, self.num_keys, self.head_size
        )

        # batch x num_heads x query_len x num_keys
        scores = (q_2 * smaple_k).sum(-1)

        if self.cope:
            for i in range(self.num_heads):
                scores[:, i, :, :] += self.position_encodings[i](q[:, i, :, :], scores[:, i, :, :])
